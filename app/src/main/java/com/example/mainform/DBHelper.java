...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase MyDB) {
        ...
        MyDB.execSQL("create Table Luong(MaLuong INTEGER PRIMARY KEY AUTOINCREMENT, MaNV TEXT, HoTen TEXT,TenPB TEXT, " +
                "LuongCB FLOAT, NgayNghi INTEGER, LuongTL FLOAT, Ngay INT, Thang INT, Nam INT)");

        MyDB.execSQL("create Table ChamCong(MaCC INTEGER PRIMARY KEY AUTOINCREMENT, MaNV VARCHAR, " +
                "NghiPhep DATE, NghiKhongPhep DATE, Ngay INT, Thang INT, Nam INT)");
    }
    ...
    public Boolean checkLuong(String MaNV, String ngay, String Thang, String Nam){
        SQLiteDatabase MyDB = this.getWritableDatabase();
        Cursor cursor = MyDB.rawQuery("Select MaNV from Luong where MaNV = ? and Ngay=? and Thang=? and Nam=?", new String[]{MaNV, ngay, Thang, Nam});
    ...
    Boolean insertLuong(String MaNV, String HoTen , String TenPB, Float LuongCB, Integer NgayNghi, Float LuongTL, String Ngay, String Thang, String Nam){
        SQLiteDatabase MyDB=this.getWritableDatabase();
        ContentValues contentValues= new ContentValues();
        contentValues.put("MaNV", MaNV);
        ...
        contentValues.put("LuongTL", LuongTL);
        contentValues.put("Ngay", Ngay);
        contentValues.put("Thang", Thang);
        contentValues.put("Nam", Nam);

        long result = MyDB.insert("Luong", null, contentValues);
        if(result==-1) return false;
        else
            return true;
    }
    Cursor readLuong(){
        String query = "Select MaNV, HoTen, LuongCB, LuongTL, (Ngay||'/'||Thang||'/'||Nam) as NgayLap from Luong";
        SQLiteDatabase MyDB = this.getReadableDatabase();
